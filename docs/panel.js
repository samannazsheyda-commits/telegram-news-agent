...
let queue=[],history=[],filtered=[],renderLimit=60,toastTimer=null;
...
function toast(message,actionText='',action=null,duration=5000){clearTimeout(toastTimer);const el=$('toast'),b=$('toastAction');$('toastText').textContent=message;b.textContent=actionText;b.style.display=actionText?'inline':'none';b.onclick=action||null;el.classList.add('show');toastTimer=setTimeout(()=>el.classList.remove('show'),duration)}
...
async function createCommand(payload,attempt=0){
  ...
  if(!r.ok){if(r.status===401||r.status===403)setConnection('مجوز اتصال مشکل دارد','bad');throw Error(`GitHub ${r.status}`)}
  return id;
}
...
function applyFilters(){
  ...
  filtered.sort((a,b)=>{const aa=new Date(a.published_at_source||a.updated_at||0),bb=new Date(b.published_at_source||b.updated_at||0);return order==='oldest'?aa-bb:bb-aa});
  renderQueue();
}

function cardHtml(r){
  const d=restoreDraft(r.id),title=d?.title??originalTitle(r),body=d?.body??originalBody(r),edited=!!d;
  return`<article class="card" data-id="${esc(r.id)}"><div class="card-head"><div style="flex:1"><input class="title-input" data-role="title" value="${esc(title)}" aria-label="تیتر خبر"><div class="meta"><span class="badge">${esc(r.source||'منبع')}</span><span>${fmt(r.published_at_source)}</span>${r.rejection_reason?`<span class="reason">${esc(r.rejection_reason)}</span>`:''}<span class="edited" ${edited?'':'hidden'}>ویرایش شده</span></div></div></div><textarea class="body-input" data-role="body" aria-label="متن خبر">${esc(body)}</textarea><div class="card-actions"><button class="btn green" data-action="publish">انتشار</button><button class="btn red" data-action="reject">رد</button>${r.source_url?`<a class="btn ghost" href="${esc(r.source_url)}" target="_blank" rel="noopener">منبع</a>`:''}<button class="btn ghost draft-reset" data-action="reset" ${edited?'':'hidden'}>بازگشت به متن اولیه</button></div></article>`;
}

function renderQueue(){
  const el=$('queueList');
  if(!filtered.length){el.innerHTML='<div class="empty">فعلاً خبری در صف نیست.</div>';updateStats();return}
  const visible=filtered.slice(0,renderLimit);
  el.innerHTML=visible.map(cardHtml).join('')+(filtered.length>renderLimit?`<button class="btn ghost" data-global-action="more">نمایش ${Math.min(40,filtered.length-renderLimit).toLocaleString('fa-IR')} خبر بعدی</button>`:'');
  updateStats();
}

function renderProcessing(){
  const rows=[...activeCommands.values()];
  $('processingList').innerHTML=rows.length?rows.map(x=>`<article class="card processing"><strong>${esc(x.title||originalTitle(x.item))}</strong><div class="meta"><span>${x.action==='publish'?'در حال انتشار':'در حال رد'}</span><span>${esc(x.message||'در انتظار پردازش')}</span><span>${fmt(x.startedAt)}</span></div></article>`).join(''):'<div class="empty">فرمان فعالی نیست.</div>';
}
...
function hideCard(id){hiddenItems.add(id);const card=document.querySelector(`[data-id="${CSS.escape(id)}"]`);if(card){card.classList.add('removing');setTimeout(applyFilters,180)}else applyFilters()}
...
function optimisticReject(item){
  hideCard(item.id);
  return new Promise(resolve=>{
    const state={done:false};
    const finish=value=>{if(state.done)return;state.done=true;clearTimeout(state.timer);pendingRejects.delete(item.id);resolve(value)};
    state.timer=setTimeout(()=>finish(true),4500);
    pendingRejects.set(item.id,state);
    toast('خبر فوراً از صف کنار رفت','برگردان',()=>{restoreRejectedCard(item);finish(false)},4500);
  });
}
...
async function continueWatching(cmd){
  const result=await pollCommandResult(cmd,300000);
  if(result.status==='timeout'){const pending=activeCommands.get(cmd);if(pending){pending.message='هنوز در حال بررسی؛ وضعیت را دوباره چک کن';renderProcessing()}return}
  await reconcileCommandResult(cmd,result);
}

async function reconcileCommandResult(cmd,result){
  const pending=activeCommands.get(cmd);if(!pending)return;
  if(result.status==='timeout'){pending.message='زمان‌بر شده؛ پردازش ادامه دارد';renderProcessing();toast('پردازش طول کشیده؛ خبر دوباره وارد صف نشده است');void continueWatching(cmd);return}
  activeCommands.delete(cmd);
  if(result.status==='failed'){
    if(pending.action==='publish')restorePublishCard(pending);else restoreRejectedCard(pending.item);
    toast((pending.action==='publish'?'انتشار':'رد')+' ناموفق بود؛ خبر به صف برگشت');
  }else{
    clearDraft(pending.item.id);
    toast(result.status==='reconciled'?'وضعیت خبر با کانال هماهنگ شد':(pending.action==='publish'?'انتشار در تلگرام تأیید شد':'خبر رد شد'));
    await load(false);
  }
  renderProcessing();updateStats();
}

async function rejectItem(item){
  if(pendingRejects.has(item.id))return;
  const proceed=await optimisticReject(item);if(!proceed)return;
  const provisional=commandId();
  activeCommands.set(provisional,{action:'reject',item,title:originalTitle(item),body:'',message:'فرمان رد ثبت می‌شود…',startedAt:new Date().toISOString()});renderProcessing();updateStats();
  try{
    const actual=await createCommand({command_id:provisional,action:'reject',item_id:item.id,title:'',body:''});rekeyActiveCommand(provisional,actual);
    const pending=activeCommands.get(actual);if(pending){pending.message='در حال نهایی‌کردن رد';renderProcessing()}
    await reconcileCommandResult(actual,await pollCommandResult(actual));
  }catch(e){activeCommands.delete(provisional);restoreRejectedCard(item);renderProcessing();toast('خطا در ثبت رد: '+e.message)}
}

async function publishItem(item,card){
  const title=card.querySelector('[data-role="title"]').value.trim(),body=card.querySelector('[data-role="body"]').value.trim();
  if(!title){card.querySelector('[data-role="title"]').focus();toast('تیتر نمی‌تواند خالی باشد');return}
  if(!item.source_url){toast('لینک منبع برای انتشار لازم است');return}
  const provisional=commandId();optimisticPublish(item,title,body,provisional);
  try{
    const actual=await createCommand({command_id:provisional,action:'publish',item_id:item.id,title,body});rekeyActiveCommand(provisional,actual);
    const pending=activeCommands.get(actual);if(pending){pending.message='در انتظار تأیید تلگرام';renderProcessing()}
    await reconcileCommandResult(actual,await pollCommandResult(actual));
  }catch(e){const pending=activeCommands.get(provisional);activeCommands.delete(provisional);if(pending)restorePublishCard(pending);renderProcessing();toast('خطا در ثبت انتشار: '+e.message)}
}
...
async function load(showLoading=true){
  try{
    ...
    const[q,h]=await Promise.all([getJson(`${RAW}/data/editorial_queue.json`),getJson(`${RAW}/data/editorial_history.json`)]);
    queue=Array.isArray(q)?q:[];history=Array.isArray(h)?h:[];reconcileHiddenWithServer();populateFilters();applyFilters();renderHistoryViews();$('staleBadge').hidden=true;$('lastRefresh').textContent='بروزرسانی '+new Intl.DateTimeFormat('fa-IR',{timeStyle:'medium'}).format(new Date());
  }catch(e){toast('داده تازه دریافت نشد؛ نمایش قبلی حفظ شد');$('staleBadge').hidden=false}
}
...
document.addEventListener('click',e=>{
  const tab=e.target.closest('[data-view]');if(tab){document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x===tab));document.querySelectorAll('.view').forEach(x=>x.classList.toggle('active',x.id===`view-${tab.dataset.view}`));if(tab.dataset.view==='system')loadSystem();return}
  const more=e.target.closest('[data-global-action="more"]');if(more){renderLimit+=40;renderQueue();return}
  const action=e.target.closest('[data-action]');if(!action)return;const card=action.closest('.card[data-id]'),item=queue.find(x=>x.id===card?.dataset.id);if(!item)return;if(action.dataset.action==='publish')publishItem(item,card);if(action.dataset.action==='reject')rejectItem(item);if(action.dataset.action==='reset')resetDraft(item,card);
});
