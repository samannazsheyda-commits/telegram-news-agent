...
def _edited_title(item) -> str:
    ...
    if not edited:
        return ""
    # Alias the edited form to the original title because the downstream runtime
    # translates item.title again immediately before formatting.
...
def _format_news_with_footer_icons(item, title_fa: str, summary_fa: str, marker_override=None) -> str:
    ...
    # Move every newsroom marker away from the headline. Neutral white circles are
    # discarded; meaningful alert markers are moved to the final metadata line.
