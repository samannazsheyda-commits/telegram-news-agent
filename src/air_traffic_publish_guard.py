...
def publish_fresh_air_traffic_snapshot(
    *,
    ...
    aircraft = reference.fetch_live_aircraft()
    visible = reference.filter_visible_aircraft(aircraft)
    if len(visible) < reference.MIN_PUBLISH_AIRCRAFT:
        raise RuntimeError(
            f"insufficient live aircraft for safe publication: {len(visible)} < {reference.MIN_PUBLISH_AIRCRAFT}"
        )
    ...
    print("AIR_TRAFFIC_PUBLISHED fresh_map=1", flush=True)
    return path
