...
if ! id bikhabar >/dev/null 2>&1; then useradd --system --create-home --home-dir /var/lib/bikhabar --shell /usr/sbin/nologin bikhabar; fi
...
seed_once "${APP_DIR}/data/editorial_queue.json" "${RUNTIME_DATA}/editorial_queue.json"
seed_once "${APP_DIR}/data/editorial_history.json" "${RUNTIME_DATA}/editorial_history.json"
...
for pair in \
  ...
  key="${pair%%=*}"; value="${pair#*=}"
  if grep -q "^${key}=" "${ENV_DIR}/agent.env"; then sed -i "s#^${key}=.*#${key}=${value}#" "${ENV_DIR}/agent.env"; else echo "${key}=${value}" >> "${ENV_DIR}/agent.env"; fi
done
...
systemctl enable bikhabar-agent bikhabar-panel bikhabar-editor bikhabar-deploy.timer bikhabar-weather.timer bikhabar-air-traffic.timer >/dev/null
...
if systemctl is-active --quiet bikhabar-agent; then echo "AGENT=active"; else echo "AGENT=not-started (set TELEGRAM_BOT_TOKEN in ${ENV_DIR}/agent.env)"; fi
if systemctl is-active --quiet bikhabar-editor; then echo "EDITOR=active"; else echo "EDITOR=not-started (shares TELEGRAM_BOT_TOKEN)"; fi
