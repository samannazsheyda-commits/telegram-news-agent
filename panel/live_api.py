...
def _clean_persian_title(value: str) -> str:
    ...
    for pattern, replacement in LATIN_REPLACEMENTS:
        text = pattern.sub(replacement, text)
    # Default RTL rows must not contain raw English glosses. The untouched
    # source title is exposed separately behind the original-text expander.
...
def _primary_news_candidate(title: str, persian_title: str = "") -> bool:
    ...
    if "?" in clean or "؟" in clean:
        return False
    if any(re.search(pattern, lower, re.I) for pattern in ARTICLE_PATTERNS):
        return False
    if any(re.search(pattern, lower, re.I) for pattern in TEASER_PATTERNS):
        return False
    words = re.findall(r"[\w\u0600-\u06ff]+", clean, flags=re.UNICODE)
    if len(words) < 4 and not any(term in lower for term in EVENT_TERMS):
        return False
    return True
...
def _fast_rows(limit: int = 80) -> list[dict]:
    ...
    for row in rows:
        ...
        result.append({
            ...
            # Compatibility with the previous client during rolling deploy.
