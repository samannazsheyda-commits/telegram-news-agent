...
from src.runtime_v6 import _format_news_with_bottom_flags
...
def test_x_post_text_drops_leading_alert_emoji_and_urls():
    ...
    assert clean_visible_x_text(raw) == "BREAKING: Iranian media report blasts near Kharg Island."
...
def test_x_post_text_drops_trailing_live_updates_promo():
    ...
    assert clean_visible_x_text(raw) == "BREAKING: An Iranian tanker was hit."
...
def test_country_flags_are_below_source_link_and_above_brand_footer():
    ...
    text = _format_news_with_bottom_flags(
        item,
