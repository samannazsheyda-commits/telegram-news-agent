...
def _store(tmp_path):
    ...
    item = ReviewItem.for_news(
        news_key="news-1", source="Test", source_url="https://example.com/story-1",
        original_title="Original", original_summary="Summary", persian_title="تیتر", persian_body="متن",
    )
...
def _command(tmp_path, *, action="publish", item_id="", title="تیتر", body="متن", command_id="cmd-1"):
    ...
    path.write_text(json.dumps({
        "command_id": command_id, "action": action, "item_id": item_id, "title": title, "body": body,
...
def _raw(item_id: str, title: str) -> RawNewsItem:
    return RawNewsItem(
        source="Reuters", source_url=f"https://example.com/{item_id}", source_item_id=item_id,
        published_at="2026-09-07T12:00:00+00:00", fetched_at="2026-09-07T12:01:00+00:00", title=title,
    )
...
def test_load_command_args_preserves_persian_and_newlines(tmp_path):
    ...
    path.write_text(json.dumps({
        "command_id": "cmd-1", "action": "publish", "item_id": "abc123",
        "title": "تیتر \"نهایی\"", "body": "خط اول\nخط دوم", "created_at": "2026-09-07T08:00:00+00:00",
    }, ensure_ascii=False), encoding="utf-8")
...
def test_refresh_command_does_not_require_item_and_runs_scanner(tmp_path):
    ...
    result = process_command_file(
        str(command), store=LocalEditorialStore(tmp_path / "queue.json", tmp_path / "history.json"),
        token="bot", chat_id="@bikhabaar", state_path=str(tmp_path / "state.json"),
        result_dir=str(tmp_path / "results"), refresh_runner=lambda: calls.append("ran") or 0,
    )
    assert calls == ["ran"]
...
def test_v2_panel_refresh_populates_live_feed_without_filling_pending_when_auto_publish_on(tmp_path):
    ...
    items = [
        _raw("one", "Iran launches ballistic missiles toward Israel"),
        _raw("two", "Jordan intercepts Iranian missiles over its airspace"),
    ]
    summary = scan_items_into_v2_panel(
        items, store=store, ledger=ledger, live_feed=live,
        settings={"auto_publish": True, "freshness_hours": 3}, now=datetime(2026, 9, 7, 12, 30, tzinfo=timezone.utc),
    )
...
def test_v2_panel_refresh_queues_when_auto_publish_is_off(tmp_path):
    store = LocalEditorialStore(tmp_path / "queue.json", tmp_path / "history.json")
    summary = scan_items_into_v2_panel(
        [_raw("manual", "Iran launches a new missile toward Israel")], store=store,
        ledger=EventLedger(tmp_path / "ledger.json"), live_feed=LiveFeedStore(tmp_path / "live.json"),
        settings={"auto_publish": False, "freshness_hours": 3}, now=datetime(2026, 9, 7, 12, 30, tzinfo=timezone.utc),
    )
...
def test_publish_retry_never_sends_telegram_twice(tmp_path):
    ...
    sent = []
    result_dir = tmp_path / "results"
    def sender(text, token, chat_id): sent.append(text)
...
def test_publish_failure_keeps_item_pending_and_writes_failed_result(tmp_path):
    store, item = _store(tmp_path)
    command = _command(tmp_path, item_id=item.id)
    def boom(*args, **kwargs): raise RuntimeError("telegram down")
