...
import src.runtime_v3 as runtime_v3
...
def test_all_distinct_iran_related_x_posts_survive_editorial_prefilters(monkeypatch):
    ...
    old = _x("old", "Iran sanctions update")
    ...
    monkeypatch.setattr(base, "_recent_published_items", lambda: [old])
    monkeypatch.setattr(base, "_is_recent_duplicate", lambda item, refs: True)

    runtime_v3.install_x_publish_all_policy()
